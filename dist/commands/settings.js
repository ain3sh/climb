...
export async function settingsMenuInteractive(config) {
    ...
    while (true) {
        try {
            ...
            settingsChoices.push({ value: 'cache', name: `⏱️  Edit Cache Settings (structure ${currentConfig.cacheTTL.structure / 1000}s, output ${currentConfig.cacheTTL.output / 1000}s)`, description: 'Configure cache TTL values' }, { value: 'theme', name: `🎨 Edit Theme (color: ${currentConfig.theme.primaryColor}, colors: ${currentConfig.theme.enableColors ? 'on' : 'off'})`, description: 'Customize colors and appearance' }, { value: 'timeout', name: `⏲️  Edit Timeouts (default ${currentConfig.timeout.default / 1000}s, introspection ${currentConfig.timeout.introspection / 1000}s, execute ${currentConfig.timeout.execute / 1000}s)`, description: 'Configure operation timeouts' }, { value: 'reset', name: '🔄 Reset to Defaults', description: 'Restore default settings' }, { value: 'back', name: '← Back', description: 'Return to main menu' });
